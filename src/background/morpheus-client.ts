import { MORPHEUS_API_URL, MORPHEUS_MODEL } from '../shared/constants'
import type { ToolDefinition, StreamCallbacks, ToolCallAccumulated } from './chutes-client'

export type { ToolCallAccumulated }
...
async function getApiKey(): Promise<string> {
  const result = await chrome.storage.sync.get('morpheus_api_key')
  if (!result.morpheus_api_key) throw new Error('No Morpheus API key configured. Please open Settings.')
  return result.morpheus_api_key as string
}
...
async function _streamCompletion(
  ...
  const response = await fetch(MORPHEUS_API_URL, {
    method: 'POST',
    headers: {
  ...
  if (!response.ok) {
    ...
    callbacks.onError(new Error(`Morpheus API error ${response.status}: ${errText}`))
    return
  }
...
export async function chatCompletion(
  ...
  if (!response.ok) {
    const errText = await response.text()
    throw new Error(`Morpheus API error ${response.status}: ${errText}`)
  }
